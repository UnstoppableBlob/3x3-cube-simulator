...
const RubiksCube = () => {
  ...
  // Move cube state into React state
  ...
  // Calculate faces based on current cube state
  const getFaces = (cube) => {
    const faces = Array(6).fill().map(() => Array(3).fill().map(() => Array(3).fill('')));
  ...
  const scrambleCube = async () => {
    const turns = [turnRed, turnBlue, turnOrange, turnGreen, turnWhite, turnYellow];
    const numMoves = 30; // Number of random moves for scramble
    let currentCubeState = [...cubeState];
    
    for (let i = 0; i < numMoves; i++) {
      await new Promise(resolve => setTimeout(resolve, 100)); // 100ms delay between moves
      const randomTurn = turns[Math.floor(Math.random() * turns.length)];
      // Randomly decide between clockwise and counterclockwise
      const turnToApply = Math.random() < 0.5 ? randomTurn : randomTurn.map(([a, b]) => [b, a]);
      
      // Apply the turn to our tracked state
      const newCubeState = [...currentCubeState];
      turnToApply.forEach(turn => {
        newCubeState[turn[1]] = currentCubeState[turn[0]];
      });
      
      // Update both our tracking variable and the React state
